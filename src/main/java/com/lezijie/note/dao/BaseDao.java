...
* 更新操作（增，删，改）
* 查询操作
* 1.查询一个字段（只会返回一条记录只有一个字段；常用场景；查询总数量）
* 2.查询集合
* 3.查询某个对象
* */
public class BaseDao {
    ...
    * */
    ...
    /*
    * 查询一个字段（只会返回一条记录只有一个字段；常用场景；查询总数量）
    * 1.得到数据库链接
    * 2，定义sql语句
    * 3.预编译
    * 4.如果有参数，则设置参数，下标从1开始(数组或集合、循环设置参数）
    ...
    * * */
    ...
    /*
    * 查询集合(实体类JavaBean中的字段与数据库表中的字段对应
    * 1.获取数据库连接
    * 2.定义SQL语句
    * 3，预编译
    * 4.如果有参数，则设置参数，下标从1开始(数组或集合、循环设置参数）
    * 5.执行查询，得到结果集
    * 6.得到结果集的原数据对象（查询到的字段数量以及查询了解那些字段
    * 7.判断并分析结果集
    * 8.实例化对象
    * 9.遍历查询字段数量，得到数据库中查询到的每一个列名
    ...
    * 15.关闭资源*/
    public static List queryRows(String sql,List<Object>params,Class cls){
        ...
        try{
            ...
            //得到结果集的原数据对象（查询到的字段数量以及查询了解那些字段
